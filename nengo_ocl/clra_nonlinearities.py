...
% endfor
    ...
    if blockify:
        ...
        Istarts = get_starts(inputs.itervalues())
        Ostarts = get_starts(outputs.itervalues())
        Pstarts = get_starts(params.itervalues())
        Pshape0s = [
            to_device(queue, x.shape0s[inds]) for x in params.itervalues()]

        lsize = None
        gsize = (block_size, len(sizes))

        full_args = []
        for vstarts, v in zip(Istarts, inputs.itervalues()):
            full_args.extend([vstarts, v.cl_buf])
        for vstarts, v in zip(Ostarts, outputs.itervalues()):
            full_args.extend([vstarts, v.cl_buf])
        for vstarts, vshape0s, v in zip(Pstarts, Pshape0s, params.itervalues()):
